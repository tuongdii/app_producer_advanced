...
class Register(QMainWindow):
    ...
    def register(self):
        ...
        name = self.ui.txtFullName.text()
        email = self.ui.txtEmail.text()
        password = self.ui.txtPassword.text()
        
        # Kiểm tra các trường thông tin có được nhập hay không
        if not name:
            self.msg_box.setText("Vui lòng nhập tên!")
            self.msg_box.exec()
            return
        if not email:
            self.msg_box.setText("Vui lòng nhập email hoặc số điện thoại!")
        ...
        if not self.ui.checkBox.isChecked():
            self.msg_box.setText("Vui lòng đọc và đồng ý các điều khoản của AdoptMate!")
            self.msg_box.exec()
            return
    ...
    def emit_register_signal(self):
        username = self.ui.txtFullName.text()
